...
async function renderQrPage(opts: { pin: string; controlUrl: string; serverHost: string; version?: string; hostname?: string; ip?: string; wifi?: string; ethernet?: string }) {
  const qr = await QRCode.toDataURL(opts.controlUrl, {
    width: 320, margin: 2, color: { dark: '#000', light: '#fff' },
  });
  const infoRows: string[] = [];
  if (opts.hostname) infoRows.push(`<div><span class="ik">host</span> <span class="iv">${escHtml(opts.hostname)}</span></div>`);
  if (opts.ip)       infoRows.push(`<div><span class="ik">ip</span> <span class="iv">${escHtml(opts.ip)}</span></div>`);
  if (opts.wifi)     infoRows.push(`<div><span class="ik">wifi</span> <span class="iv">${escHtml(opts.wifi)}</span></div>`);
  else if (opts.ethernet) infoRows.push(`<div><span class="ik">net</span> <span class="iv">${escHtml(opts.ethernet)} (wired)</span></div>`);
...
export function registerRoutes(app: Application) {
  ...
  app.get('/qr', async (req, res) => {
    ...
    const hostname = req.query.hostname ? String(req.query.hostname).slice(0, 64) : undefined;
    const ip       = req.query.ip       ? String(req.query.ip).slice(0, 64)       : undefined;
    const wifi     = req.query.wifi     ? String(req.query.wifi).slice(0, 64)     : undefined;
    const ethernet = req.query.ethernet ? String(req.query.ethernet).slice(0, 32) : undefined;
    res.send(await renderQrPage({ pin, controlUrl, serverHost, version, hostname, ip, wifi, ethernet }));
  });
